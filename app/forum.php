...
class Forum {
	...
	public $topics, $posts, $view, $layout, $db, $fw, $isAuth;

	function __construct() {
		$this->fw = Base::instance();

                $this->db = new DB\SQL($this->fw->get('dburi'),
                        $this->fw->get('dbuser'),
                        $this->fw->get('dbpasswd')
                        );
	...
	function listTopics() {
		...
		/* pagination stuff */
		$topicsPerPage = $this->fw->get('forum.topics.perpage');
		...
		//$posts = new DB\Mongo\Mapper($this->db, 'posts');
                ...
                $topics = new DB\SQL\Mapper($db, 'topics');
		$activeTopics = $topics->paginate($pageNumber, $topicsPerPage, $condition, $options);

		if (count($activeTopics['subset']) === 0) {
			$this->fw->push('SESSION.messages.error', 'The requested page wasn\'t found');
			$this->fw->reroute('/page/' . ($activeTopics['count'] || 1));
		}
